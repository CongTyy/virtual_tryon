...
class WCT(nn.Module):
    ...
    def wct(self, c_A, s_B): 
        ...
        print(f'{c_A_mean.size()}, {c_A_.size()}, {cov_c.size()},')
        ...
        print(f'{whitend.size()}, {colored_B.size()}')
...
if __name__ == "__main__":
    ...
    C = 256
    n_mem = C // 8
    mask = get_block_diagonal_mask(n_mem).cuda()
    # model = Generator(64, 8, mask, 8, 256, 256, 256, 'cpu')

    # x = torch.rand((1, 3, 256, 192)).cuda()

    # ce = Content_Encoder(64, 8//2, norm = 'in', activation='relu').cuda()   
    # se = Style_Encoder(64, 8, norm = 'gn', activation='relu').cuda()   
    model = Decoder(256, mask, n_group=8, bias_dim=256, mlp_dim=256, repeat_num=8//2, norm='ln', device='cuda').cuda()

    ce = torch.rand((1, 256, 64, 48)).cuda()
    se = torch.rand((1, 256, 1, 1)).cuda()
    summary(model, input_data = [ce, se])